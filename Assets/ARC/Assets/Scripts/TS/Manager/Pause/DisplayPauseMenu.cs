...
namespace TS.Generics
{
    public class DisplayPauseMenu : MonoBehaviour
    {
        ...
        bool IsPauseAllowed()
        {
            ...
            if (howManyPlayer > 1)
            {
                if (!InfoPlayerTS.instance.returnCheckState(0) ||
                  (LapCounterAndPosition.instance && LapCounterAndPosition.instance.posList.Count > 0 && LapCounterAndPosition.instance.posList[0].IsRaceComplete &&
                    LapCounterAndPosition.instance.posList[1].IsRaceComplete))
                    return false;
            }
            else
            {
                if (!InfoPlayerTS.instance.returnCheckState(0) || (LapCounterAndPosition.instance && LapCounterAndPosition.instance.posList.Count > 0 && LapCounterAndPosition.instance.posList[0].IsRaceComplete))
                    return false;
            }
        ...
        public void MusicMenuPause(int musicID)
        {
            ...
            if (MusicManager.instance.ListAudioSource.Count > currentAudioSource && MusicManager.instance.ListAudioSource[currentAudioSource].time > 0)
