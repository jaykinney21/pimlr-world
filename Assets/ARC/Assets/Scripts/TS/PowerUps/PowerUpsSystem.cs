    ...
    public class PowerUpsSystem : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            for (var i = 0; i < listLayersUsedBylayerMaskPowerDetection.Count; i++)
            {
                ...
                for (int j = 0; j < LayersRef.instance.layersListData.listLayerInfo.Count; j++)
                {
                    if (LayersRef.instance.layersListData.listLayerInfo[j].layerID == listLayersUsedBylayerMaskPowerDetection[i])
                    {
                        layerUsed[i] = LayerMask.LayerToName(LayersRef.instance.layersListData.listLayerInfo[j].layerID);
                    }
                }
            }
