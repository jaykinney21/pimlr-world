    ...
    public class F3DBeam : MonoBehaviour
    {
        ...
        void Raycast()
        {
            ...
            if (Physics.Raycast(ray, out hitPoint, MaxBeamLength, layerMask))
            {
                ...
                if (f3DFXController != null)
                {
                    switch (fxType)
                    {
                        case F3DFXType.Sniper:
                            f3DFXController.SniperImpact(hitPoint.point + hitPoint.normal * fxOffset);
                            ApplyForce(4f);
                            break;

                        case F3DFXType.RailGun:
                            f3DFXController.RailgunImpact(hitPoint.point + hitPoint.normal * fxOffset);
                            ApplyForce(7f);
                            break;
            ...
            else
            {
                ...
                if (ray2D)
                {
                    ...
                    {
                        case F3DFXType.Sniper:
                            f3DFXController.SniperImpact(ray2D.point + ray2D.normal * fxOffset);
                            //ApplyForce(4f);
                            break;

                        case F3DFXType.RailGun:
                            //f3DFXController.RailgunImpact(ray2D.point + ray2D.normal * fxOffset);
                            ApplyForce(7f);
                            break;
        ...
        public void ColliderWithObject(GameObject other)
        {
            if (f3DFXController == null)
            {
                f3DFXController = FindObjectOfType<F3DFXController>();
            }
            if (other.CompareTag("Enemy"))
            {
                ...
                if (playerIDComponent != null && playerIDComponent.playerID != SocketNetworkManager.Instance.playerID&& f3DFXController.playerid  && f3DFXController.playerid.isLocalPlayer)
                {
