...
namespace FORGE3D
{
    public class F3DTurret : MonoBehaviour
    {
        ...
        internal Transform barrelTransform;
        ...
        [SerializeField] F3DFXController f3DFXController;
        private void Awake()
        {
            headTransform = Swivel.transform;
            barrelTransform = Mount.transform;
        ...
        {
            if (this.gameObject.activeSelf && (f3DFXController&& f3DFXController.playerid && f3DFXController.playerid.isLocalPlayer || (f3DFXController&& f3DFXController.playerid && f3DFXController.playerid.isAIPlayer && SocketPlayerManager.Instance.helixPlayerInfo.isRoomHostUser && false)))
                SocketNetworkManager.Instance.EmitMachineGunRot(f3DFXController.playerid.playerID, targetPos, headTransform.rotation.eulerAngles);
        }


        public void UpdateRotation(Vector3 newPos, Vector3 newRot)
        {
            //barrelTransform.rotation.eulerAngles = newRot;

            targetPos = newPos;
            headTransform.eulerAngles = newRot;
        ...
        private void Update()
        {
            ...
            if ((f3DFXController!=null &&f3DFXController.playerid && f3DFXController.playerid.isLocalPlayer) || DebugTarget != null)
            {
                isRotate = true;
                if (DebugTarget != null)
                ...
                {
                    barrelX = barrelTransform;
                    barrelY = Swivel.transform;
