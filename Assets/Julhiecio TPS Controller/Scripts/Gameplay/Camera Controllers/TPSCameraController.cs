...
namespace JUTPS.CameraSystems
{
	...
	public class TPSCameraController : JUCameraController
	{
		...
		public CameraState FireModeCameraState = new CameraState("Fire Mode Camera State",movementSpeed: 0);
		//public CameraState FireModeCameraState = new CameraState("Fire Mode Camera State",1.5f,50f,50f,1f,0,0,0.5f,0.5f,0,1f,-80f,80);
		...
		public void CameraUpTargetOffset()
		{
			
            if (characterTarget.DriveVehicleAbility.IsDriving)
            {
				//FireModeCameraState.ForwardCameraOffset = 100;
				DrivingVehicleCameraState.UpTargetOffset = 1f;
				//characterTarget.FiringModeIK = false;

			}
            else
            {
				FireModeCameraState.UpTargetOffset = 0f;
			}
        }
