...
namespace JUTPS.CameraSystems
{
	...
	public class TDCameraController : JUCameraController
	{
		...
		protected virtual void UpdateCharacterState(JUCharacterController character)
		{
			...
			if (!character.IsAiming && !character.IsDriving && !character.FiringMode && !character.IsDead && !character.IsDriveByShooting) { CharacterState = PlayerStates.Normal; }
			...
			if (character.IsDriveByShooting) { CharacterState = PlayerStates.FireMode; }
		}
