...
using UnityEngine.SceneManagement; // Add this line at the top of your script
...
namespace JUTPS.WeaponSystem
{
	...
	public class Weapon : HoldableItem
	{
		...
		public void Shot()
		{
			...
			// Raycasting to find out what the player is shooting at
			RaycastHit hit;
			if (Physics.Raycast(transform.position, transform.forward, out hit))
			{
				// Check if the player hit a GameObject tagged "Enemy"
				if (hit.collider.gameObject.CompareTag("Enemy"))
				{
					Debug.Log("Hit an enemy!");

					// Schedule the LoadNewScene method to be called after a 2 seconds delay
		...
		void LoadNewScene()
		{
			...
			//SceneManager.LoadScene("Music Player Landscape");
		}
