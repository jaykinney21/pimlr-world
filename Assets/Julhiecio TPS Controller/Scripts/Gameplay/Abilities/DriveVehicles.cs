...
namespace JUTPS.ActionScripts
{
    ...
    public class DriveVehicles : JUTPSActions.JUTPSAction
    {
        ...
        public Vehicle VehicleToShootWithDrive;
        ...
        // PIMLR #12: When entering a vehicle the character used to unequip the held item
        // (water gun) via SwitchToItem(-1). Keep the gun equipped by default; set true to
        // restore the original "holster on enter" behaviour.
        [SerializeField] private bool UnequipWeaponOnEnter = false;

        [Header("States")]
        public bool VehicleDrivableNearby;
        public bool IsDriving;
        public bool IsDrivingByShoot;
        ...
        protected virtual void Update()
        {
            ...
            if (JUInput.GetButtonDown(JUInput.Buttons.EnterVehicleButton))
            {
                ...
                else if (IsDrivingByShoot)
                {
                    ExitVehicle();
                }
                else
                {
                    SceneManagerScript.Instance.minimapBlipController.playerCarBlip.gameObject.SetActive(false);
                    //Debug.Log("----------DriveVehicle----------");
                    DriveVehicle();
                }
            }

            if (IsDriving && !hasInvoked)
            {
                Invoke("ActivateObject", 5f);
                //IsDriving = false; // To ensure the Invoke is called only once
                hasInvoked = true;
            }
            
            //changes by game
            if (IsDrivingByShoot && !hasInvoked)
            {
                Invoke("ActivateObject", 5f);
                //IsDriving = false; // To ensure the Invoke is called only once
                hasInvoked = true;
        ...
        public void DriveVehicle()
        {
            ...
            Debug.LogError("policeCar::" + this.gameObject.name);
            //GameExecutionManager.Instance.policeCar.enabled = true;
        }
        ...
        protected virtual void OnJUTPSCharacterStartDriving()
        {
            ...
            //Deselect Item
            // PIMLR #12: only unequip the held weapon on enter if explicitly opted-in.
            if (UnequipWeaponOnEnter) TPSCharacter.SwitchToItem(-1);
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            if (other.transform.tag == "VehicleShootingArea" && IsDrivingByShoot == false)
            {
                VehicleToShootWithDrive = other.GetComponentInParent<Vehicle>();
                if (VehicleToShootWithDrive != null)
                {
