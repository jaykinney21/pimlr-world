...
namespace JUTPS
{
    ...
    public class JUCharacterController : JUCharacterBrain
    {
        ...
        public virtual void ControllerInputs()
        {
            ...
            bool ShotInput = JUInput.GetButton(JUInput.Buttons.ShotButton) && !IsPointerOverUIObject();
            //bool ShotInputUp = JUInput.GetButtonUp(JUInput.Buttons.ShotButton);
            bool ShotInputDown = JUInput.GetButtonDown(JUInput.Buttons.ShotButton)&& !IsPointerOverUIObject();
            ...
            // >>> Firing Mode Trigger
            if ((ShotInput || AimInput) && FiringMode == false && IsRolling == false && IsReloading == false && LocomotionMode != MovementMode.JuTpsClassic)
            {
        ...
        protected virtual void SetupDefaultLayersWeights()
        {
            ...
            {
                //Debug.Log($"<color=cyan>JU Charater IsDriving ={IsDriving}</color>");
                bool EnabledCondition = (!IsRolling && !IsReloading);
        ...
        void OnTriggerEnter(Collider other)
        {
            ...
            //changes by game //changes by meet
            if (other.transform.tag == "VehicleShootingArea" && IsDriving == false)
            {
                
                VehicleInArea = other.GetComponentInParent<JUTPS.VehicleSystem.Vehicle>();
                IsDriveByShooting = true;
            }
        ...
        void OnTriggerExit(Collider other)
        {
            ...
            //changes by game
            if (other.transform.tag == "VehicleShootingArea" && IsDriveByShooting == true)
            {
                VehicleInShootingArea = null;
                IsDriveByShooting = false;
            }
        }
