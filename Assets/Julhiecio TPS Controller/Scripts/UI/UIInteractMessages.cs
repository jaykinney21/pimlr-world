...
namespace JUTPS.UI
{
    public class UIInteractMessages : MonoBehaviour
    {
        ...
        [Header("Vehicle Shooting Enter Message")]
        [SerializeField] private string VehicleShootingEnterLabelText = "TO Shooting";
        [SerializeField] private Vector3 VehicleShootingOffset;
        ...
        void Update()
        {
            ...
            // >> Vehicle Shooting Message
            if(JUGameManager.InstancedPlayer.VehicleInShootingArea != null && JUGameManager.InstancedPlayer.IsDriveByShooting == false)
            {
                //Debug.Log("Vehicle Shooting Message");
                PickUpMessageObject.SetActive(true);
                UIElementToWorldPosition.SetUIWorldPosition(PickUpMessageObject, JUGameManager.InstancedPlayer.VehicleInShootingArea.transform.position, VehicleShootingOffset);
                if (WarningText)
                {
                    WarningText.text = VehicleShootingEnterLabelText;
                }
                return;
            }
