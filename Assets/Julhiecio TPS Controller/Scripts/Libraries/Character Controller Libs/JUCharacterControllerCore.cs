...
namespace JUTPS.CharacterBrain
{
    ...
    public class JUCharacterBrain : MonoBehaviour
    {
        ...
        // PIMLR #11: Stamina UI hooks. SprintSpeedDecrease is the internal "stamina pool":
        // it ramps up to MaxSprintSpeedDecrease (10) while sprinting then tapers back to 0.
        // Expose it read-only + a normalized 0..1 value so a UI Image.fillAmount can show it.
        // CanSprint exposed read-only so the bar can grey-out when stamina is depleted.
        public const float MaxSprintSpeedDecrease = 10f;
        public float CurrentSprintStamina => SprintSpeedDecrease;
        public float NormalizedSprintStamina => Mathf.Clamp01(SprintSpeedDecrease / MaxSprintSpeedDecrease);
        public bool CanSprintNow => CanSprint;
        ...
        public Vehicle VehicleInShootingArea;
        ...
        protected virtual void Awake()
        {
            ...
            // PIMLR #13: the player's punches deal 5 damage (hands only; enemies + water gun keep their defaults).
            if (CompareTag("Player"))
            {
                if (RightHandDamager != null) RightHandDamager.Damage = 5f;
                if (LeftHandDamager != null) LeftHandDamager.Damage = 5f;
            }
        ...
        protected virtual void UseRightHandItem(bool ShotInput, bool ShotDownInput)
        {
            ...
            if (IsDriving && currentHeldItem == -1) return;
            ...
            if (HoldableItemInUseRightHand.ContinuousUseItem)
            {
                if (ShotInput)
                {
                    if (IsRolling == false && HoldableItemInUseRightHand.CanUseItem)
                    {
                        UseEquipedItem();
                    }
                    else
        ...
        protected virtual void UseLeftHandItem(bool ShotInput, bool ShotDownInput)
        {
            ...
            if (HoldableItemInUseLeftHand.ContinuousUseItem)
            {
                if (ShotInput)
                {
                    if (IsRolling == false && ArmsWeightIK > 0.7f && HoldableItemInUseLeftHand.CanUseItem)
                    {
                        UseEquipedItem();
                    }
                    else
            ...
            else
            {
                ...
                {
                    if (IsRolling == false && ArmsWeightIK > 0.7f && canUseItem)
                    {
                        UseEquipedItem();
                    }
                    else
        ...
        public virtual void UseWeaponRightHand(bool ShotInput, bool ShotInputDown, bool AimInput, bool AimInputDown)
        {
            ...
            if (!FiringMode || IsRolling || IsDead || IsReloading)
            {
                IsAiming = false;
                return;
            }
            ...
            else
            {
                ...
                if (ShotInputDown && IsRolling == false && ArmsWeightIK > 0.4f && WeaponInUseRightHand.BulletsAmounts > 0 && WeaponInUseRightHand.IsUsingItem == true && WeaponInUseRightHand.CurrentFireRateToShoot > 0.09f)
                {
        ...
        public virtual void UseWeaponLeftHand(bool ShotInput, bool ShotInputDown, bool AimInput, bool AimInputDown)
        {
            ...
            else
            {
                ...
                //Force shooting out of firerate
                if (ShotInputDown && IsRolling == false && ArmsWeightIK > 0.4f && WeaponInUseLeftHand.BulletsAmounts > 0 && WeaponInUseLeftHand.IsUsingItem == true && WeaponInUseLeftHand.CurrentFireRateToShoot > 0.09f)
                {
        ...
        protected void DrivingCheck()
        {
            ...
            //changes by game
            if (DriveVehicleAbility == null) { IsDriveByShooting = false; return; }
            IsDriveByShooting = DriveVehicleAbility.IsDrivingByShoot;
            VehicleInShootingArea = DriveVehicleAbility.VehicleToShootWithDrive;
        }
        ...
        public void SwitchToItem(int id = -1, bool RightHand = true)
        {
            ...
            if (JUPauseGame.Paused || IsReloading || IsReloading || IsDead  || IsDriveByShooting || IsRagdolled || DisableAllMove) return;
    ...
    public class JUAnimatorParameters
    {
        ...
        public string DriveByShooting = "DriveByShooting";
