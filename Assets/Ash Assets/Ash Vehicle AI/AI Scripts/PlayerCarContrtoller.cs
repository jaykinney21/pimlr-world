...
namespace AshVP
{
	...
	public class PlayerCarContrtoller : MonoBehaviour
	{
		public bool thisPlayer = false;
		...
		public PlayerControlSensors sensorScript;
   ...
   //         {
        ...
        void FixedUpdate ()
		{
			...
			//inputs
			float turnInput = 0;
			if (Input.GetKey(KeyCode.A)) //CHANGED
			{
				turnInput = turnTorque * -sensorScript.turnmultiplyer * Time.fixedDeltaTime * 1000;
			}
			else if (Input.GetKey(KeyCode.D)) //CHANGED
            {
				turnInput = turnTorque * sensorScript.turnmultiplyer * Time.fixedDeltaTime * 1000;
			}
			else
			{
				while  (turnInput != 0)
				{
                    turnInput = turnInput > 1 ? turnInput - sensorScript.turnmultiplyer * Time.fixedDeltaTime :
                    turnInput = turnInput < -1 ? turnInput + sensorScript.turnmultiplyer * Time.fixedDeltaTime : 0;
                }
			...
			if(Input.GetKey(KeyCode.W)) //CHANGED
            {
                 speedInput = accelerationForce* SpeedAI * Time.fixedDeltaTime * 1000;
            }
			else if(Input.GetKey(KeyCode.S))  //CHANGED
            {
				if(speedInput > 0)
				{
            ...
            else //ADDED
            {
                if (speedInput > 0)
                {
                    brakeInput = (brakeForce) * Time.fixedDeltaTime * 1000;

                    brakeInput *= Mathf.Clamp01(carVelocity.magnitude);
                }
                if (carVelocity.x < 4 && carVelocity.x > -3 &&
                   carVelocity.y < 4 && carVelocity.y > -3 &&
   ...
   //             if(speedInput > 0)
			//	{
   //                 speedInput -= accelerationForce * SpeedAI * 2 * Time.fixedDeltaTime * 1000;
   //             }
