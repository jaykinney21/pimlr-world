...
namespace AshVP
{
	...
	public class AiCarContrtoller : MonoBehaviour
	{
        ...
        private float fricValue, turnValue, curveVelocity, brakeInput;
		[HideInInspector]
        ...
        {
            // Calculate the rotation angle of the needle based on the current speed.
            //float speedPercentage = Mathf.Clamp01(speed);
            float speedPercentage = Mathf.Clamp(speed/1000,0f,160f);
			//Debug.Log("speedPercentage:" + speedPercentage);
            //float needleAngle = Mathf.Lerp(needleMinAngle, needleMaxAngle, speedPercentage);

            // Update the rotation of the needle.
            //needleTransform.rotation = Quaternion.Euler(0f, 0f, needleAngle);
        }
