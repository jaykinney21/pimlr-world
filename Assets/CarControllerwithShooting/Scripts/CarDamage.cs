...
public class CarDamage : MonoBehaviour 
{
	...
	public float demolutionRange = 0.5f; // make sure that the scale of render meshes in the mesh list is 1,1,1. If the scale is large then you will have to adjust demolutionrange, however that is not recommended. 
    ...
    void Repair()
    {
        Debug.Log("Repairing car mesh to original state.");
	...
	{
        Debug.Log($"Collision detected with {collision.gameObject.name} at velocity {collision.relativeVelocity}");
        ...
        Debug.Log($"Collision Strength: {colStrength}");
	...
	public void OnMeshForce( Vector3 originPos, float force )
	{
        ...
        for(int j=0; j<meshfilters.Length; ++j)
        {
            ...
            // Log initial position of the first vertex for debugging
            if(verts.Length > 0) Debug.Log($"Initial position of first vertex (Mesh {j}): {verts[0]}");
            ...
            // Log final position of the first vertex for debugging
            if(verts.Length > 0) Debug.Log($"Final position of first vertex (Mesh {j}): {verts[0]}");
        }
	} 
}
